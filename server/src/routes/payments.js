...
router.post('/checkout', auth, async (req, res) => {
  ...
  const receiver = await db.findOne('users', u => u.id === receiver_id);
...
router.post('/webhook', async (req, res) => {
  ...
  if (event.type === 'checkout.session.completed') {
    ...
    if (!sender_id || !receiver_id) return res.status(400).json({ error: 'Missing metadata' });

    const txId = uuid();
    await db.insert('transactions', {
      id: txId, hangout_id: hangout_id || null,
      sender_id, receiver_id,
      amount: parseFloat(amount), message: message || '',
      status: 'paid', stripe_id: session.id, created_at: Date.now(),
    });
    ...
    if (io) {
      ...
      if (sid) {
        io.to(sid).emit('round-sent', { amount: parseFloat(amount), message, sender_name, receiver_name });
        if (hangout_id) io.to(hangout_id).emit('round-sent', { amount: parseFloat(amount), message, sender_name, receiver_name });
      }
    }
    ...
    if (!userSockets[receiver_id]) {
      sendPush(receiver_id, {
        ...
        icon: '/icon-192.png', tag: `round-${txId}`, url: '/home',
      });
    }
...
router.get('/history', auth, async (req, res) => {
  const txns = await db.find('transactions', t =>
    t.sender_id === req.user.id || t.receiver_id === req.user.id
  );
    ...
    txns
      ...
      .map(async t => {
        const [sender, receiver] = await Promise.all([
          db.findOne('users', u => u.id === t.sender_id),
          db.findOne('users', u => u.id === t.receiver_id),
        ]);
        return {
          ...t,
          sender_name:   sender?.name,   sender_color:   sender?.avatar_color,
          receiver_name: receiver?.name, receiver_color: receiver?.avatar_color,
        };
      })
  );
  res.json(history);
