...
module.exports = (io) => {
  ...
  io.on('connection', async (socket) => {
    ...
    socket.on('get-friend-presence', async () => {
      const map = {};
      const friendIds = await getFriendIds(uid);
      friendIds.forEach(fid => { map[fid] = presence[fid] || 'offline'; });
        ...
        friendIds.forEach(fid => {
          if (!userSockets[fid]) {
            sendPush(fid, {
              ...
              icon: '/icon-192.png', tag: `free-${uid}`, url: '/home',
            });
          }
        });
      }
    ...
    socket.on('join-hangout', (hangoutId) => {
      ...
      hangoutRooms[hangoutId][uid] = {
        name: socket.user.name, color: socket.user.avatar_color, socketId: socket.id,
      };
      ...
      socket.to(hangoutId).emit('peer-joined', {
        userId: uid, name: socket.user.name,
        color: socket.user.avatar_color, socketId: socket.id,
      });
    });

    socket.on('hangout-ended', async ({ hangoutId }) => {
      const hangout = await db.findOne('hangouts', h => h.id === hangoutId);
      if (!hangout) return;
    ...
    socket.on('invite-to-hangout', async ({ hangoutId, targetUserId }) => {
      ...
      if (!guestIds.includes(targetUserId)) {
        await db.update('hangouts', h => h.id === hangoutId, { guest_ids: [...guestIds, targetUserId] });
      }
      ...
      if (targetSid) {
        ...
        sendPush(targetUserId, {
          ...
          icon: '/icon-192.png', tag: `invite-${hangoutId}`, url: `/hangout/${hangoutId}`,
        });
      }
    });
    ...
    // ── Chat ───────────────────────────────────────────────────────
    socket.on('chat-message', async ({ hangoutId, content }) => {
      if (!content || typeof content !== 'string' || content.trim().length === 0) return;
      if (content.length > 4000) return;
      const msg = {
        id: uuid(), hangout_id: hangoutId, sender_id: uid,
        sender_name: socket.user.name, content: content.trim(),
        type: 'text', created_at: Date.now(),
      };
    ...
    socket.on('watch-stop',    ({ hangoutId }) => socket.to(hangoutId).emit('watch-stop', { by: socket.user.name }));
